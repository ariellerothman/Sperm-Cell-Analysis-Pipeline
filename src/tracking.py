...
def wide_to_long(input_csv: str, output_csv: str, num_tracks: int = None) -> None:
    ...
    Args:
        ...
        num_tracks: Total number of tracks to expect. If None, auto-detects from column headers.
    """
    df = pd.read_csv(input_csv)
    records = []
    
    # Auto-detect number of tracks if not provided
    if num_tracks is None:
        x_cols = [col for col in df.columns if col.startswith('X') and col[1:].isdigit()]
        num_tracks = len(x_cols) if x_cols else 0
...
def visualize_tracking(
    ...
    save_overlays: bool = True, output_dir: str = None,
    verbose: bool = False, organelle_name: str = None
) -> str:
    ...
    with track IDs. All frames with valid tracking data are displayed.
    ...
    Args:
        ...
        verbose: If True, print status messages. If False, silently save overlays.
        organelle_name: Optional organelle name for overlay directory. If None, extracted from parent folder.
    ...
    # Get all unique frames with valid data
    frames_with_tracks = sorted(df['Frame'].unique().astype(int))[:frames_to_display]
    ...
    if save_overlays:
        ...
        # Determine organelle name for overlay subdirectory
        if organelle_name is None:
            # Try to infer from parent folder name (e.g., "Mito tracking" -> "mitochondria")
            parent_dir = os.path.basename(os.path.dirname(csv_path))
            if "mito" in parent_dir.lower():
                organelle_name = "mitochondria"
            elif "mo" in parent_dir.lower():
                organelle_name = "MO"
            else:
    ...
    for frame_num in frames_with_tracks:
        ...
        # Close figure without displaying (plt.show() removed to prevent notebook clutter)
        plt.close(fig)
...
def run_tracking_pipeline(
    cell_number: int, base_dir: str, organelle: str = "MO", total_tracks: int = 400, verbose: bool = False
) -> str:
    ...
    if tiff_in and os.path.exists(tiff_in):
        visualize_tracking(tiff_in, long_csv, frames_to_display=io.imread(tiff_in).shape[0], verbose=verbose)
