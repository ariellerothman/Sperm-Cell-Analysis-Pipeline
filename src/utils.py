...
def get_centroid(tif_path: str) -> np.ndarray:
    ...
    # Return centroid of largest component (by area)
    largest = max(props, key=lambda r: r.area)
    return np.array(largest.centroid)

def log_file_paths(sperm_id: int, file_paths: dict, registered: bool = False) -> list:
    """Log all file paths used for a sperm cell analysis.
    
    Args:
        sperm_id: Sperm cell ID
        file_paths: Dictionary from get_file_paths()
        registered: Whether registered versions were used
    
    Returns:
        List of log messages
    """
    logs = []
    logs.append(f"  Sperm {sperm_id} file paths (registered={registered}):")
    
    # Single organelles
    for org in ["pseudopod", "nucleus", "sperm_cell"]:
        if org in file_paths:
            filename = os.path.basename(file_paths[org])
            logs.append(f"    ✓ {org:15s}: {filename}")
    ...
    # Tracking CSVs
    for org in ["mito_csv", "mo_csv"]:
        if org in file_paths:
            filename = os.path.basename(file_paths[org])
            org_display = "MO tracking" if "mo" in org else "Mito tracking"
            logs.append(f"    ✓ {org_display:15s}: {filename}")
