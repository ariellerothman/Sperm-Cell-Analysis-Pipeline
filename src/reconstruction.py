...
from .config import (
    ...
    MESH_DECIMATE_RATIO, VIDEO_ZOOM_FACTOR, VIDEO_NUM_FRAMES, VIDEO_FPS
)

def decimate_mesh(vertices: np.ndarray, faces: np.ndarray, ratio: float = 0.7) -> tuple:
    """Reduce mesh polygon count using PyVista decimation.
    
    Args:
        vertices: Array of shape (N_verts, 3) with XYZ coordinates.
        faces: Array of shape (N_faces, 3) with vertex indices.
        ratio: Target ratio of triangles to KEEP (0.0-1.0).
                0.7 = keep 70% of triangles, remove 30%.
    
    Returns:
        Tuple of (decimated_vertices, decimated_faces).
    
    Note: PyVista's decimate() expects target_reduction (fraction to REMOVE),
    so we invert: target_reduction = 1.0 - ratio.
    """
    if ratio >= 1.0 or faces.shape[0] < 100:
        return vertices, faces  # Skip decimation if already small
    
    # Convert to PyVista format
    faces_pv = np.hstack([np.full((faces.shape[0], 1), 3), faces]).astype(np.int64)
    mesh = pv.PolyData(vertices, faces_pv)
    
    # Decimate: PyVista's target_reduction is fraction to REMOVE, not to keep
    target_reduction = 1.0 - ratio  # Invert to match PyVista semantics
    decimated = mesh.decimate(target_reduction)
    
    # Extract back to standard format
    dec_verts = decimated.points
    dec_faces = decimated.faces.reshape(-1, 4)[:, 1:]  # Remove cell count column
    
    return dec_verts, dec_faces
...
def build_3d_scene(
    ...
    sperm_mask_path: str = None, decimate_ratio: float = MESH_DECIMATE_RATIO
) -> pv.Plotter:
        ...
        try:
            ...
            if v is not None and f is not None:
                ...
                if decimate_ratio < 1.0:
                    v_orig_count = v.shape[0]
                    v, f = decimate_mesh(v, f, decimate_ratio)
                    print(f"  {o['name']}: {v_orig_count} → {v.shape[0]} verts ({v.shape[0]/v_orig_count*100:.1f}%), {f.shape[0]} faces")
                else:
